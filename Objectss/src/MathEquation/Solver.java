...
public class Solver{
 ...
 "|" +
	...
	public static String getNoTricks(String usersInput) {
		...
		return "Woops, we couldn't interpret that.";
	}
	/**
	...
	public static void solve(Equation eq) {
		ArrayList<Term> sideWithVariable=new ArrayList<Term>();
		gui.clearSteps();//make sure gui is a static field
		if(eq.cancelRight() == true){
			eq.toZeroOnOneSide(eq.getRightSide());
			gui.addStep(eq,"1");
			sideWithVariable=eq.getLeftSide();
		}
		else {
			eq.toZeroOnOneSide(eq.getLeftSide());
			gui.addStep(eq,"2");
			sideWithVariable=eq.getRightSide();
		}
		...
		if(eq.isLinear() ==true){
			eq.solveLinear(sideWithVariable);
			gui.addStep(eq,"scalar multi");
		
		
			System.out.println(eq);
			gui.addStep(eq,"Answer");
		}
		
	}	
	
	public static double getRoot1(ArrayList<Term> side){
		double a = 0;
		double b = 0;
		double c = 0;
		for(Term x : side)
		...
		return (-b - Math.sqrt(getDiscriminate(a,b,c))) / (2*a);
	}
	
	public static double getRoot2(ArrayList<Term> side){
		double a = 0;
		double b = 0;
		double c = 0;
		for(Term x : side)
